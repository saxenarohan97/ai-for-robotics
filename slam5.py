...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    omegax = matrix([[0. for i in range(N + 1 + num_landmarks)] for j in range(N + 1 + num_landmarks)])
    omegay = matrix([[0. for i in range(N + 1 + num_landmarks)] for j in range(N + 1 + num_landmarks)])
    omega = [omegax, omegay]

    xix = [[0.] for j in range(N + 1 + num_landmarks)]
    xiy = [[0.] for j in range(N + 1 + num_landmarks)]
    xi = [xix, xiy]

    init = [world_size/2., world_size/2.]

    #initial location constraint
    for x in range(2):
        omega[x].value[0][0] += (1./measurement_noise)
        xi[x].value[0][0] += (1./measurement_noise) * init[x]

    #motion constraint
    for x in range(2):
        for i in range(N):
            
            omega[x].value[i+1][i+1] += (1./motion_noise)

            omega[x].value[i+1][i] -= (1./motion_noise)

            xi[x].value[i+1][0] += distance

            omega[x].value[i][i] += (1./motion_noise)

            omega[x].value[i][i+1] -= (1./motion_noise)

            xi[x].value[i][0] -= distance
