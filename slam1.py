...
class robot:
    ...
    # --------
    # sense: 
    #    simulating the sensing by a robot by adding some noise
...
def run(grid, goal, spath, params, printflag = False, speed = 0.1, timeout = 1000):
    ...
    while not myrobot.check_goal(goal) and N < timeout and (index + 1) < len(spath):
        ...
        cte = ((estimate[1] - spath[index][1])*(spath[index+1][0] - spath[index][0]) - (estimate[0]-spath[index][0])*(spath[index+1][1] - spath[index][1])) / ((spath[index+1][0] - spath[index][0])**2 + (spath[index+1][1] - spath[index][1])**2)
        ...
        u = ((estimate[0]-spath[index][0])*(spath[index+1][0] - spath[index][0]) + (estimate[1] - spath[index][1])*(spath[index+1][1] - spath[index][1])) / ((spath[index+1][0] - spath[index][0])**2 + (spath[index+1][1] - spath[index][1])**2)
